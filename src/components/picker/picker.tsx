...
export interface Props {
  items: PickerItem[];
  name?: string;
  defaultSelected?: PickerItem[];
  placeholder?: string;
  onChange?: (selected: PickerItem[]) => any;
}
...
class Picker extends React.Component<Props, State> {
  ...
  handleSelect = (item: PickerItem) => {
    const { selected } = this.state;

    const indexOf = selected
      .map((item: PickerItem) => item.value)
      .indexOf(item.value);
    ...
    return (
      ...
      >
        ...
        <HiddenSelect
          ...
          multiple
          onChange={() => {}}
          value={this.state.selected.map((item: PickerItem) => item.value)}
        >
